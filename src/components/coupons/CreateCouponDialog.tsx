...
export default function CreateCouponDialog(props: PropsDialog) {
    ...
    const [result, callAPI] = usePost<Coupon>({
        url: config.ADMIN_API + "/coupons",
        headers: authHeader()
    });
    ...
    const createCoupon = handleSubmit<Omit<Coupon, 'id'>>(input => {
        ...
        callAPI(value as Coupon).then(res => {
            if (res.error) {
            ...
            setSeverity('success')
            setMsg("Coupon added successfuly");
            setMsgOpen(true);
            loadData();
            onClose();
