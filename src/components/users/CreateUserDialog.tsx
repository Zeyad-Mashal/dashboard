...
export default function CreateUserDialog(props: PropsDialog) {
    ...
    const [result, callAPI] = usePost<User>({
        url: config.ADMIN_API + "/users",
        headers: authHeader()
    });
    ...
    const createUser = handleSubmit<Omit<User, 'id'>>(input => {
        ...
        callAPI(value as User).then(res => {
            if (res.error) {
