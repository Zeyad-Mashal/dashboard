...
export default function CreateDeliveryAgentDialog(props: PropsDialog) {
    ...
    const [result, callAPI] = usePost<DeliveryAgent>({
        url: config.ADMIN_API + "/delivery-agents",
        headers: authHeader()
    });
    ...
    const createDeliveryAgent = handleSubmit<Omit<DeliveryAgent, 'id'>>(input => {
        ...
        callAPI(value as DeliveryAgent).then(res => {
            if (res.error) {
            ...
            setSeverity('success')
            setMsg("Delivery Agent added successfuly");
            setMsgOpen(true);
            loadData();
            onClose();
